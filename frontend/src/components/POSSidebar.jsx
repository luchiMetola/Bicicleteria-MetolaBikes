...
import { Home, Package, Wrench, User, LogOut, ChevronLeft, ChevronRight, Truck, Bell, BellOff, X, BellRing } from 'lucide-react';
...
function POSSidebar({isSidebarOpen, setIsSidebarOpen}) {
  ...
  return (
    <aside className={`fixed top-0 left-0 h-full bg-slate-950 text-white p-4 shadow-xl transition-all duration-300 z-40 flex flex-col justify-between
      ...
      <div className="space-y-6">
          ...
          {mostrarModalAlertas && (
            <div className="absolute top-0 left-full ml-4 z-50 w-72 bg-white border border-slate-200 text-slate-800 rounded-2xl shadow-2xl p-4 animate-in fade-in slide-in-from-left-2 duration-150">
              <div className="flex justify-between items-center border-b border-slate-100 pb-3 mb-3">
                <span className="font-black text-[11px] uppercase tracking-wider text-slate-500">Nuevos Ingresos</span>
              ...
              <div className="space-y-2 max-h-60 overflow-y-auto pr-1">
                {alertas.map((alerta) => (
                  <div 
                    key={alerta.id} 
                    onClick={() => redirigirPorTipo(alerta.tipo)}
                    className={`p-3 rounded-xl text-xs font-medium leading-relaxed border shadow-xs cursor-pointer transition-transform hover:scale-[1.02] ${
                      alerta.tipo === 'alerta_stock' ? 'bg-amber-50 border-amber-200 text-amber-900' : 'bg-emerald-50 border-emerald-200 text-emerald-900'
                    }`}
                  >
                    {alerta.mensaje}
                  </div>
                ...
                {alertas.length === 0 && (
                  <p className="text-slate-400 text-center text-xs font-semibold py-6">No hay alertas operativas.</p>
                )}
              </div>
        ...
        <nav className="flex flex-col gap-1.5 w-full mt-2">
          <Link to="/" className={linkClase('/')}>
            ...
            {isSidebarOpen && <span>Caja Mostrador</span>}
          </Link>
          ...
          <Link to="/workshop" className={linkClase('/workshop')}>
            ...
            {isSidebarOpen && <span>Taller & Mecánica</span>}
          </Link>

          {/* AÑADIDO: Acceso al Historial de Notificaciones */}
          <Link to="/notifications" className={linkClase('/notifications')}>
            <BellRing className="w-5 h-5 shrink-0" />
            {isSidebarOpen && <span>Bandeja de Avisos</span>}
          </Link>
