...
import { Wrench, Calendar, Clock, CheckCircle, AlertCircle, ListFilter, MapPin, CalendarOff } from 'lucide-react';
...
function Workshop({ userName }) {
  ...
  // ESTADOS NUEVOS DE CONTROL DE DISPONIBILIDAD GLOBAL
  ...
  const handleRequestAppointment = async (e) => {
    ...
    if (!tallerHabilitado) {
      setNotification({ text: 'La agenda del taller no está recibiendo solicitudes en este momento.', type: 'error' });
      return;
    }

    if (codigoPostal.trim() !== '5400') {
      setNotification({ text: 'El servicio de taller solo está disponible para residentes de la localidad de San Juan (CP: 5400).', type: 'error' });
      return;
    }
    ...
    try {
      ...
      setNotification({ text: error.response?.data?.error || 'No se pudo reservar el turno.', type: 'error' });
    } finally {
  ...
  return (
    <div className="min-h-screen bg-slate-50 font-sans p-4 md:p-8 w-full transition-all">
      ...
      {/* AVISO DEL ESTADO DEL TALLER (SI ESTÁ CON MUCHA DEMORA O APAGADO) */}
      {!tallerHabilitado && (
        <div className="mb-6 bg-rose-50 border border-rose-200 rounded-2xl p-4 flex items-start gap-3 shadow-xs">
          <CalendarOff className="w-5 h-5 text-rose-600 shrink-0 mt-0.5" />
          <div>
            <h4 className="text-sm font-bold text-rose-800">Agenda Web Desactivada por Alta Demanda</h4>
            <p className="text-xs text-rose-700 mt-1 leading-relaxed font-semibold">
              📢 Mensaje del Taller: "{mensajeEstado || 'Temporalmente saturado. Solo atención presencial.'}"
            </p>
          </div>
        </div>
      )}
        ...
        <div className="xl:col-span-1 bg-white border border-slate-200 rounded-3xl p-5 shadow-xs">
          ...
          <form onSubmit={handleRequestAppointment} className="space-y-4 text-xs font-medium text-slate-600">
            <div>
              ...
              <input type="text" required disabled={!tallerHabilitado} value={bikeModel} onChange={(e) => setBiciModelo(e.target.value)} placeholder="Ej: Venzo Raptor Rodado 29" className="w-full p-2.5 border border-slate-300 rounded-xl text-slate-800 disabled:bg-slate-100 disabled:text-slate-400" />
            </div>

            <div>
              <label className="block text-[11px] font-bold uppercase mb-1">Diagnóstico / Detalle del Service:</label>
              <textarea rows="2" required disabled={!tallerHabilitado} value={description} onChange={(e) => setDescripcion(e.target.value)} placeholder="Ej: Ajuste de cambios y centrado de llanta..." className="w-full p-2.5 border border-slate-300 rounded-xl text-slate-800 resize-none disabled:bg-slate-100 disabled:text-slate-400" />
            </div>
            ...
            <div>
              ...
              <input type="text" required disabled={!tallerHabilitado} value={codigoPostal} onChange={(e) => setCodigoPostal(e.target.value)} placeholder="Ej: 5400" className="w-full p-2.5 border border-slate-300 rounded-xl text-slate-800 font-semibold disabled:bg-slate-100 disabled:text-slate-400" />
            </div>
            ...
            <div>
              ...
              <input type="date" required disabled={!tallerHabilitado} min={getTodayDateString()} value={selectedDate} onChange={(e) => setSelectedDate(e.target.value)} className="w-full p-2.5 border border-slate-300 rounded-xl text-slate-800 font-semibold disabled:bg-slate-100 disabled:text-slate-400" />
            </div>

            {selectedDate && tallerHabilitado && (
            ...
            <button 
              type="submit" 
              disabled={loadingSubmit || !selectedTime || !tallerHabilitado} 
              className="w-full py-3 bg-blue-600 hover:bg-blue-700 disabled:opacity-40 disabled:hover:bg-blue-600 disabled:cursor-not-allowed text-white font-bold rounded-xl shadow-md transition-colors cursor-pointer text-xs uppercase tracking-wider"
            >
              {loadingSubmit ? 'Procesando...' : !tallerHabilitado ? 'Agenda Suspendida' : 'Solicitar Turno en Taller'}
            </button>
          </form>
        </div>
